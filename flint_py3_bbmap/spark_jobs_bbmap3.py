...
def profile_sample(sampleReadsRDD, sc, ssc, output_file, save_to_s3, save_to_local, sensitive_align, partition_size,
    ...
    #
    def align_with_bbmap(iterator):
        """
        Function that runs on ALL worker nodes (Executors). Dispatches a BBMap command and handles read alignments.
        Args:
        ...
        try:
            align_subprocess = sp.Popen(bbmapCMD, stdin=sp.PIPE, stdout=sp.PIPE, stderr=sp.PIPE,bufsize=1)
            alignment_output, alignment_error = align_subprocess.communicate(input="\n".join(reads_list).encode("latin-1"))
    ...
    #
    print("trying main function of profileSample()")
    try:
        print("Reached after the try statement")
        if not sampleReadsRDD.isEmpty():
            print("Reached main profileSample function lines 621")
            if get_shard_counter() == 0:
                print("first iteration into getshardcounter")
            ...
            #
            #   Dispatch the Alignment job with BBMap
            #
            print("[" + time.strftime('%d-%b-%Y %H:%M:%S', time.localtime()) +
                  "] Aligning reads with BBMap (" + str(bowtie2_number_threads) + ")...")
            ...
            print("[" + time.strftime('%d-%b-%Y %H:%M:%S', time.localtime()) + "] BBMap - Complete. " +
            ...
            if debug_mode:
                ...
                print("Checkpoint 7... strain level map")
                print(checkpoint_7)
                chk_7_e = time.time()
                print("[" + time.strftime('%d-%b-%Y %H:%M:%S', time.localtime()) +
            ...
            if debug_mode:
                ...
                print("Checkpoint 8... Abundance strains:")
                print(checkpoint_8)
                chk_8_e = time.time()
                print("[" + time.strftime('%d-%b-%Y %H:%M:%S', time.localtime()) +
            ...
            if coalesce_output:
                ...
                print("collect strain abundances....")
                ...
                # making sure that we have the correct amount in memory
                print("Line 922:.......")
                print(abundance_count)

            else:
                print("Going into shard splitting")
                if save_to_s3:
            ...
            print("Getting shard counter before increment")
            print(get_shard_counter())
            increment_rdd_count()
            print("Getting shard counter after increment")
            print(get_shard_counter())
